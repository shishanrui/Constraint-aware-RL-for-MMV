...
from .safety import SafetyDecision, SafetyShield
from .signals import (
    ACTION_SPACE_NVECS,
    CO2_SETPOINT_PPM,
    CSV_COL_DATETIME,
...
class MMVEnv(gym.Env):
    ...
    def __init__(
        ...
        apply_safety_overrides: bool = True,
    ):
        super().__init__()
        self.month = month
        ...
        self.apply_safety_overrides = bool(apply_safety_overrides)
    ...
    def step(self, action):
        ...
        if self.apply_safety_overrides:
            ...
            decision = self._raw_action_decision(
                requested_mode=int(requested_mode),
                requested_sp_k=float(requested_sp_k),
            )
    ...
    def _raw_action_decision(self, *, requested_mode: int, requested_sp_k: float) -> SafetyDecision:
        req = int(requested_mode)
        req_sp_k = float(requested_sp_k)

        if req == MODE_OFF:
            u_sys_on = 0.0
            u_win_ope = 0.0
            troo_set = float(self.default_ac_sp_k)
        elif req == MODE_NV:
            u_sys_on = 0.0
            u_win_ope = 1.0
            troo_set = float(self.default_ac_sp_k)
