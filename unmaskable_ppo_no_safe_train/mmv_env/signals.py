...
def decode_ac_setpoint_index(sp_idx: int) -> float:
    """Decode the setpoint branch independently of the requested mode."""
    sp_i = int(sp_idx)
    if not 0 <= sp_i < N_AC_SP:
        raise ValueError(f"Invalid AC setpoint action index: {sp_idx}")
    return float(AC_SP_VALUES_C[sp_i]) + 273.15
...
def decode_action(mode_idx: int, sp_idx: int):
    ...
    - NV  -> AC setpoint is retained as a comfort reference for the low-level
             fan controller
    ...
    if mode in {MODE_NV, MODE_AC}:
        return mode, decode_ac_setpoint_index(sp_i)
