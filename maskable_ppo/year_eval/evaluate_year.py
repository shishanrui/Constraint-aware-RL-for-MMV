...
from scripts.run_config import infer_run_config_from_model_name, with_overrides
...
def main() -> None:
    ...
    parser.add_argument("--out-csv", type=str, default=str(CFG.output_csv), help="Output CSV path")
    ...
    train_cfg_for_model = infer_run_config_from_model_name(model_path.stem, base_cfg=CFG.train_cfg)
    eval_env_cfg = with_overrides(
        train_cfg_for_model,
        dt_comm_s=CFG.eval_env_cfg.dt_comm_s,
        month=CFG.eval_env_cfg.month,
    )
    ...
    print(f"[YEAR-EVAL] Using dt_comm_s={eval_env_cfg.dt_comm_s}")
    print(
        "[YEAR-EVAL] Using reward config "
        f"we={train_cfg_for_model.w_energy}, wpmv={train_cfg_for_model.w_pmv}, "
        f"ws={train_cfg_for_model.w_switch}, nv={train_cfg_for_model.nv_bonus_beta}, "
        f"nvu={train_cfg_for_model.nv_bonus_unocc_beta}, wnvu={train_cfg_for_model.w_nv_unbenefit}"
    )
    model = MaskablePPO.load(str(model_path), device="cpu")

    def _make_env_for_vecnorm():
        return make_eval_env(
            train_cfg=eval_env_cfg,
            fmu_path=fmu_path,
        ...
        writer.writerow(
            {
                ...
                "comfort_penalty": float(last_comfort_penalty),
                "energy_penalty": float(last_energy_penalty),
                ...
                "override_penalty": float(last_override_penalty),
                "nv_bonus": float(last_nv_bonus),
    ...
    with output_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        while (float(env.backend.time) - eval_t0_s) < t_end_s - 1e-9:
            ...
            requested_sp_k = float(env.default_ac_sp_k) if requested_sp_k is None else float(requested_sp_k)
            ...
            decision = env.safety.decide(
                ...
                requested_troo_set_k=requested_sp_k if int(requested_mode) == MODE_AC else None,
            ...
            last_comfort_penalty = float(rb.comfort_penalty)
            last_energy_penalty = float(rb.energy_penalty)
            ...
            last_override_penalty = float(rb.override_penalty)
            last_nv_bonus = float(rb.nv_bonus)
