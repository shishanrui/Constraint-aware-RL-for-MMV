...
class MMVEnv(gym.Env):
    ...
    def __init__(
        ...
        self.backend = FMUBackend(
            FMUBackendConfig(
                ...
                quiet_stderr_during_step=False,  # True
        ...
        self._mask_occ_fra = 0.0
        self._mask_rain_mm = 0.0
        self._mask_wind_m_s = 0.0
        self._mask_tout_k = float(self.default_ac_sp_k)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self._update_action_mask_state(
            occ_fra=float(occ),
    ...
    def step(self, action):
        ...
        requested_sp_k = float(self.default_ac_sp_k) if requested_sp_k is None else float(requested_sp_k)
        ...
        self._update_action_mask_state(
            occ_fra=float(occ_next),
            rain_mm=float(rain_next),
            wind_m_s=float(outputs["WinFmu"]),
            tout_k=float(outputs["Tout"]),
        )
    ...
    def action_masks(self) -> np.ndarray:
        nv_allowed = self._nv_allowed(
            rain_mm=float(self._mask_rain_mm),
            wind_m_s=float(self._mask_wind_m_s),
            tout_k=float(self._mask_tout_k),
            current_mode=int(self.current_mode),
            time_since_ac_on_s=float(self.time_since_ac_on_s),
        ) >= 0.5
        occupied = float(self._mask_occ_fra) >= OCC_ON_THRESHOLD
        ac_hold = (int(self.current_mode) == MODE_AC) and (float(self.time_since_ac_on_s) < MIN_AC_ON_S)
        if ac_hold:
            # Treat the minimum AC-on window as a hard operational constraint.
            mode_mask = np.array([False, False, True], dtype=bool)
        else:
            mode_mask = np.array([True, nv_allowed, occupied], dtype=bool)
        sp_mask = np.ones(int(self.action_space.nvec[1]), dtype=bool)
        return np.concatenate([mode_mask, sp_mask])
    ...
    def _update_action_mask_state(
        ...
        *,
        ...
        self._mask_occ_fra = float(occ_fra)
        self._mask_rain_mm = float(rain_mm)
        self._mask_wind_m_s = float(wind_m_s)
        self._mask_tout_k = float(tout_k)
