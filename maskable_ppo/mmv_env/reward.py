...
class RewardCalculator:
    def __init__(
        ...
        *,
        ...
        self.w_nv_blocked = float(w_nv_blocked)
        self.w_min_ac_hold = float(w_min_ac_hold)
        self.w_unocc_ac = float(w_unocc_ac)
    ...
    def compute(
        ...
        *,
        ...
        unoccupied_ac_penalty = (
            -self.w_unocc_ac
            if occ < self.occ_on_threshold and int(mode_applied) == self.mode_ac
            else 0.0
        )
        ...
        if int(mode_requested) == self.mode_nv and overridden and self._is_nv_block_reason(override_reason):
            nv_blocked_penalty = -self.w_nv_blocked
        ...
        if overridden and str(override_reason or "") == "min_ac_on_time_hold":
            min_ac_hold_penalty = -self.w_min_ac_hold
        override_penalty = nv_blocked_penalty + min_ac_hold_penalty
